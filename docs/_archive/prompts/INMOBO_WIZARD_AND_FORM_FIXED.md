# Codex Agent Prompt â€” Fix Wizard de creaciÃ³n de Resource (antes Properties)
...
IMPORTANT: The product is multi-language, but primarily Spanish. Use UTF-8 and keep accents and â€œÃ±â€. Update i18n keys/texts accordingly.

## Context

We currently have a dynamic wizard used for creating and editing resources via:

- `/app/properties/new` (CreateProperty â†’ PropertyWizard)
- `/app/properties/:id/edit` (EditProperty using same dynamic engine)
  Even though the route says â€œpropertiesâ€, it actually supports multiple `resourceType` profiles.
...
- Pricing model labels and meaning are confusing (e.g., â€œtotalâ€).
- Some fields are missing in flows where they make sense (e.g., selling a furnished house).
- Categories overlap/confuse the mental model (e.g., â€œproperty: commercialâ€ vs â€œvenue: commercial_localâ€).
...
1. **Refactor the wizard selection logic** so that the flow is coherent and strictly filtered:
   - Step order must be: **Resource Type â†’ Category â†’ Commercial Mode** (if applicable).
...
2. **Fix semantic rules for verticals**
   ...
   - Ensure default booking types are consistent:
     - sale â†’ manual_contact
     - rent_short_term â†’ date_range
     - rent_long_term â†’ manual_contact (or explicit monthly)
   - Ensure allowed pricing models:
     - rent_short_term â†’ per_day (and only those that make sense)
     - rent_long_term â†’ per_month (optionally allow per_day only if explicitly intended; otherwise remove)
   ...
   - For properties like house/apartment/land/office/warehouse/commercial:
     ...
     - Hourly rental should not appear in `resourceType=property` flows.
   - Add **furnished** field to **sale** flows for houses/apartments/commercial/office/warehouse where it makes sense.
     - If you donâ€™t want it on land, keep it excluded.
   - Fix â€œland + rent_long_termâ€ showing furnished/petsAllowed (currently semantically weak). Decide:
   ...
   ### Venues / Services / Experiences / Music
   - Ensure categories like:
     - event_hall, commercial_local, studio, coworking, meeting_room
       belong to `resourceType=venue` for rentable spaces.
   - Ensure bookable verticals with hourly mode are consistent:
     - `service`, `experience`, `music`, `venue` can use `rent_hourly` with `bookingType=time_slot`.
     - `music` is first-class (`resourceType=music`) and `dj` belongs here, not under `service`.
...
3. **Fix pricing model UX and meaning**
   - The label â€œtotalâ€ is confusing. Define clear semantics:
     - If â€œtotalâ€ means â€œprecio fijo por operaciÃ³n/ventaâ€ â†’ rename to something like:
       - `fixed_total` (UI: â€œPrecio fijoâ€ / â€œPrecio total (fijo)â€)
     - If it means â€œprecio final calculadoâ€ â†’ it should NOT be a user-selected pricing model; itâ€™s an output of a quote.
...
4. **Wizard copy and field labels must be precise**
   - Review all â€œModo comercialâ€, â€œCategorÃ­aâ€, â€œPrecioâ€, â€œPeriodoâ€, etc.
...
6. **Documentation updates (mandatory)**
   ...
   - Add a short â€œChangelogâ€ section at the top explaining what changed and why.

7. **Testing**
   - Add/update unit tests (or lightweight integration tests) for:
     - Filtering logic (category â†’ commercialMode â†’ pricingModel)
...
2. Centralize the rules:
   - Create a single â€œcatalogâ€ / â€œprofile resolverâ€ module that outputs:
     - allowed categories
...
- â€œTotalâ€ pricing is no longer confusing; the UI shows a clear name and meaning.
- Selling a house/apartment can capture â€œamuebladoâ€ (furnished) when appropriate.
