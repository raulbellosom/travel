...
const StepVacationRules = ({ formHook }) => {
  ...
  const fields = resourceFormProfile.vacationRules;
  ...
  if (fields.length === 0) {
    return (
      <p className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
        {t("propertyForm.wizard.noSpecificBookingRules", {
          defaultValue:
            "Este tipo de recurso no requiere reglas adicionales de reserva.",
        })}
      </p>
    );
  }
